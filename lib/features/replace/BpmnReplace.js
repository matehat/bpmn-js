...
function BpmnReplace(bpmnFactory, moddle, popupMenu, replace, selection, modeling) {
  ...
  /**
   * toggles the active state of the current entry
   * @param  {Object} entry
   */
    ...
    return [
      {
        ...
        action: function(event, entry){
          if (entry.active) {
            ...
            modeling.updateProperties(element, { loopCharacteristics: { isSequential: false }});
          }
          toggleEntry(entry);
        }
      },
      {
        ...
        action: function(event, entry){
          if (entry.active) {
            ...
            modeling.updateProperties(element, { loopCharacteristics: { isSequential: true }});
          }
          toggleEntry(entry);
        }
      },
      {
        ...
        action: function(event, entry){
          if (entry.active) {
            ...
            modeling.updateProperties(element, { loopCharacteristics: { isSequential: undefined }});
          }
          toggleEntry(entry);
        }
      }
